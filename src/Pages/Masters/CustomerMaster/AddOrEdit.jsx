...
yup.addMethod(yup.array, 'uniqueMagazines', function (message) {
	return this.test('unique-magazines', message, function (list) {
		const { path, createError } = this;
		const magazineNames = new Set();
		for (let i = 0; i < list.length; i++) {
			const magazine = list[i].magazine;

			if (magazineNames.has(magazine)) {
				return createError({
					path: `${path}.${i}.magazine`,
					message: message || 'Duplicate magazine entry',
				});
			}
			magazineNames.add(magazine);
		}
		return true;
	});
});

yup.addMethod(yup.array, 'uniqueMembers', function (message) {
	return this.test('unique-members', message, function (list) {
		const { path, createError } = this;
		const memberEmails = new Set();
		for (let i = 0; i < list.length; i++) {
			const member = list[i].name;

			if (memberEmails.has(member)) {
				return createError({
					path: `${path}.${i}.name`,
					message: message || 'Duplicate member name',
				});
			}
			memberEmails.add(member);
		}
		return true;
	});
});
...
const schema = yup.object().shape({
	...
	magazines: yup.array().of(magazineSchema).uniqueMagazines('Duplicate magazine entries found'),
	members: yup.array().of(memberSchema).uniqueMembers('Duplicate member emails found'),
});
				...
				<div className="mb-4">
					<div className="flex justify-between items-center mb-2">
						...
						<Button
							type="button"
							variant="outline"
							onClick={() => appendMember({ id: 0, cid: 0, name: '', email: '', contactNo: '' })}
						>
							<Plus className="h-4 w-4 mr-2" />
							Add Member
						</Button>
