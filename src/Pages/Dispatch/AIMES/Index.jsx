...
function AIMEGeneration() {
	...
	const formSchema = yup.object().shape({
		...
		re11indent: yup
			...
			.test(
				...
				(value) => value && value !== '' && value !== 'Select' && value !== `RE-11/${year}/`,
			)
		...
		aime_no: yup
			...
			.test(
				...
				(value) => value && value !== '' && value !== 'Select' && value !== `AIME/${year}/`,
			)
		...
		routeLocations: yup
			.array()
			.of(yup.string())
			.test('all-dates-present', 'Please select a date for each route location.', function (locations) {
	...
	useEffect(() => {
		if (imtimationData) {
			const intiOptions = [...new Set(imtimationData?.map((mag) => mag.name))].sort().map((mag) => ({
				value: mag,
				text: mag,
				disabled: false,
			}));
		...
		if (routeData) {
			const routeOptions = [...new Set(routeData?.map((mag) => mag.cname))].sort().map((mag) => ({
				value: mag,
				text: mag,
				disabled: false,
			}));
						...
						<div className="grid grid-cols-1 gap-5">
							...
							<div className="flex flex-col gap-y-6">
								...
								<div className="relative my-2">
									...
									<div className="relative flex justify-center">
										<span className="bg-transparent  px-2 text-sm font-medium">
											Intimation Details
										</span>
									</div>
								</div>
										...
										<div className="flex flex-col gap-y-2">
											...
											<Select
												...
												onValueChange={(value) => {
													...
													const selected = imtimationData?.find(
														(inti) => inti.name === value,
													);
													if (selected) {
												...
												<SelectContent>
													<SelectGroup>
														{imtimationData?.map((mag) => (
															<SelectItem key={mag.name} value={mag.name}>
																{mag.name}
															</SelectItem>
														))}
													</SelectGroup>
											...
											{errors.ExposovesOffice && (
												<span className="text-destructive text-sm">
													{errors.ExposovesOffice.message}
												</span>
											)}
										</div>
									)}
							...
							<div className="space-y-2">
								...
								{errors.address_office && (
									<p className="text-red-500 text-sm">{errors.address_office.message}</p>
								)}
							</div>
											...
											<Select
												...
												onValueChange={(value) => {
													...
													if (selected) {
														...
														const routeLocations = selected.locations
															? selected.locations.split('-')
															: [];
							...
							<div className="space-y-2">
								<Label>Routes</Label>
								<Textarea {...register('route_address')} placeholder="Routes..." rows={7} readOnly />
								{errors.route_address && (
									<p className="text-red-500 text-sm">{errors.route_address.message}</p>
								)}
							</div>
