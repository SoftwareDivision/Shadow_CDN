...
function AddOrEditLoadingSheet() {
	...
	const {
		data: loadingSheetData,
		isLoading: isLoadingLoadingSheetData,
		error: loadingSheetDataError,
	} = useQuery({
		queryKey: ['loadingSheet', id],
		queryFn: () => getLoadingSheetDetailsById(tokendata, id),
		enabled: !!tokendata && isEdit,
		onError: (error) => {
			enqueueSnackbar(error.message || 'Failed to fetch loading sheet data', { variant: 'error' });
		},
	});
	...
	useEffect(() => {
		if (isEdit && loadingSheetData) {
			reset({
				mfgdt: new Date(loadingSheetData.mfgdt),
				loadingSheetNo: loadingSheetData.loadingSheetNo,
				truckNo: loadingSheetData.truckNo,
				transporterName: loadingSheetData.transporterName,
				indentToAdd: '',
			});
			setSelectedIndents(loadingSheetData.indentInfoViewModels || []);
		}
	}, [isEdit, loadingSheetData, reset, setValue]);

	useEffect(() => {
		const sourceData = isEdit ? loadingSheetData : createData;
	...
	}, [createData, loadingSheetData, watchedTransporterName, watchedTruckNo, setValue, isEdit]);
	...
	const mutation = useMutation({
		mutationFn: (data) => {
			const payload = {
				...
				indentInfoViewModels: selectedIndents,
			};
	...
	const onSubmit = (data) => {
		...
		// Call the mutation with the data after validation check
		mutation.mutate(data);
	};

	if (isLoadingCreateData || (isEdit && isLoadingLoadingSheetData)) {
		return (
	...
	if (createDataError || loadingSheetDataError) {
		return (
			<div className="text-center text-red-500">
				Error loading data: {createDataError?.message || loadingSheetDataError?.message}
			</div>
		);
	}

	const availableIndentsForSelection = isEdit ? [] : createData?.indentInfoViewModels || [];
			...
			<form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
				<div className="relative my-2">
					...
					<div className="relative flex justify-center">
						<span className="bg-background px-2 text-sm font-medium">Loading Sheet Information</span>
					</div>
				</div>
				...
				<div className="grid grid-cols-1 md:grid-cols-3 gap-5">
					...
					<div className="flex flex-col gap-y-2">
						...
						<Controller
							...
							render={({ field }) => (
								<Select onValueChange={field.onChange} value={field.value}>
									<SelectTrigger className={errors.truckNo ? 'border-red-500 w-full' : 'w-full'}>
						...
						/>
						{errors.truckNo && <span className="text-sm text-red-500">{errors.truckNo.message}</span>}
					</div>
