...
import { getPlantDetails, getl2ReprintReport, exportL2ReprintDataToExcel } from '@/lib/api';
...
function L2_Barcode_Reprint_Report() {
	...
	const onExportToExcel = async (data) => {
		...
		try {
			...
			const blob = await exportL2ReprintDataToExcel(tokendata, reportParams);
			...
			link.setAttribute(
				'download',
				`L2_Barcode_Reprint_Report_${data.reportType}_${format(new Date(), 'dd-MM-yyyy')}.xlsx`,
			);
			...
			<div>
				{reportData ? (
					<SummableDataTable
						...
						data={reportData}
						fileName={`L2_Barcode_Reprint_Report_${reportType}_${format(new Date(), 'dd-MM-yyyy')}`}
