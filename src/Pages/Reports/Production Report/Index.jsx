...
function Production_Report() {
    ...
    const formSchema = z.object({
        ...
        shiftId: z.string(),
        plantId: z.string(),
        brand: z.string().optional(), // Added brand
        productsize: z.string().optional(), // Added productsize
    });
    ...
    const {
        ...
        defaultValues: {
            ...
            productsize: '', // Default value for productsize
        },
    });
    ...
    const onSubmit = async (data) => {
        ...
        const selectedShift = data.shiftId === 'all' ? '' : data.shiftId;
                                ...
                                <Popover>
                                    ...
                                    <PopoverContent className="w-auto p-0">
                                        <Calendar
                                            mode="single"
                                            selected={fromDate}
                                            onSelect={(date) => { setFromDate(date); setValue("fromDate", date); }} // Update form value
                                            initialFocus
                                        />
                                    </PopoverContent>
                                </Popover>
                            ...
                            render={({ field }) => (
                                <div className="flex flex-col gap-y-2">
                                    ...
                                    <Select
                                        ...
                                        onValueChange={(value) => {
                                            field.onChange(value);
                                            // Reset product size when brand changes
                                            setValue('productsize', '');
                                            setProductSizes([]);

                                            if (value === 'all') {
                                                // If 'All' is selected, show no product sizes
                                                setProductSizes([]);
                                                setValue('brandId', ''); // Set brandId to empty for 'All'
                                            } else {
                                                // Otherwise, filter product sizes based on selected brand
