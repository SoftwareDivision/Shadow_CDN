...
import { getl1BoxDeletionReport, getProductDetails, exportL1BoxDataToExcel } from '@/lib/api';
...
function L1_Box_Deletion_Report() {
	...
	const onExportToExcel = async (data) => {
		...
		try {
			...
			const blob = await exportL1BoxDataToExcel(tokendata, reportParams);
			...
			link.setAttribute(
				'download',
				`L1_Box_Deletion_Report_${data.reportType}_${format(new Date(), 'dd-MM-yyyy')}.xlsx`,
			);
			...
			<div>
				{reportData ? (
					<SummableDataTable
						...
						data={reportData}
						fileName={`L1_Box_Deletion_Report_${reportType}_${format(new Date(), 'dd-MM-yyyy')}`}
