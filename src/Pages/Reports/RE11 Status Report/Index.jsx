...
import { getRe11StatusReport, getCustomerDetails, getIndentOnly, exportRE11StatusDataToExcel } from '@/lib/api';
...
function RE11_Status_Report() {
	...
	useEffect(() => {
		if (customerData) {
			const customerOptions = [...new Set(customerData?.map((customer) => customer.cName))]
				.sort()
				.map((customer) => ({
					value: customer,
					text: customer,
	...
	const onExportToExcel = async (data) => {
		...
		try {
			...
			const blob = await exportRE11StatusDataToExcel(tokendata, reportParams);
			console.log('Blob:', blob);
			// Create a download link and trigger the download
			const url = window.URL.createObjectURL(new Blob([blob]));
			const link = document.createElement('a');
			link.href = url;
			link.setAttribute('download', `RE11_Status_Report_${format(new Date(), 'dd-MM-yyyy')}.xlsx`);
			...
			<div>
				{reportData ? (
					<SummableDataTable
						...
						data={reportData}
						fileName={`RE11_Status_Report_${format(new Date(), 'dd-MM-yyyy')}`}
