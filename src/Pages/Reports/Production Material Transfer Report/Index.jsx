...
import { getPlantDetails, getPrd_Material_Trns_Report, exportProTransFetchDataToExcel } from '@/lib/api';
...
function Prd_Material_Reprint_Report() {
	...
	const onSubmit = async (data) => {
		...
		try {
			...
			const result = await getPrd_Material_Trns_Report(tokendata, reportParams);
	...
	const detailedReportColumns = [
		...
		{
			accessorKey: 'transferdt',
			header: 'Transfer Date',
			cell: ({ row }) => {
				const date = row.getValue('transferdt');
	...
	const summaryReportColumns = [
		...
		},
		{
			header: 'Brand & Size',
			cell: ({ row }) => {
				const magazine = row.original.brand;
				const size = row.original.psize;
	...
	const onExportToExcel = async (data) => {
		...
		try {
			...
			const blob = await exportProTransFetchDataToExcel(tokendata, reportParams);
			...
			link.setAttribute(
				'download',
				`Production_Material_Transfer_Report_${data.reportType}_${format(new Date(), 'dd-MM-yyyy')}.xlsx`,
			);
	...
	return (
		<Card className="p-4 shadow-md">
			<div className="flex justify-between items-center">
				<h1 className="text-2xl font-semibold">Production Material Transfer Report</h1>
			</div>
			...
			<div>
				{reportData ? (
					<SummableDataTable
						...
						data={reportData}
						fileName={`Production_Material_Transfer_Report_${reportType}_${format(
							new Date(),
							'dd-MM-yyyy',
						)}`}
...
export default Prd_Material_Reprint_Report;
