...
import { getPlantDetails, getl1ReprintReport, exportL1ReprintDataToExcel } from '@/lib/api';
...
function Production_Report() {
	...
	const onSubmit = async (data) => {
		...
		try {
			...
			const result = await getl1ReprintReport(tokendata, reportParams);
	...
	const onExportToExcel = async (data) => {
		...
		try {
			...
			const blob = await exportL1ReprintDataToExcel(tokendata, reportParams);
			...
			link.setAttribute(
				'download',
				`L1_Barcode_Reprint_Report_${data.reportType}_${format(new Date(), 'dd-MM-yyyy')}.xlsx`,
			);
	...
	return (
		<Card className="p-4 shadow-md">
			<div className="flex justify-between items-center">
				<h1 className="text-2xl font-semibold">L1 Barcode Reprint Report</h1>
			</div>
			...
			<div>
				{reportData ? (
					<SummableDataTable
						...
						data={reportData}
						fileName={`L1_Barcode_Reprint_Report_${reportType}_${format(new Date(), 'dd-MM-yyyy')}`}
