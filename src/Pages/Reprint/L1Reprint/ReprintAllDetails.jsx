...
const schema = yup.object().shape({
	...
	brandId: yup.string(), // Assuming Brand Id is derived and not directly validated
	productSize: yup.string().required('Product Size is required'),
	pSizeCode: yup.string(), // Assuming Product Size Code is derived and not directly validated
});

export default function ReprintAllDetails() {
	...
	const onSubmit = (data) => {
		...
		const payload = {
			...
			shift: data.shift, // Machine Code
			Brandname: data.brandName, // Brand Name
			BrandId: data.brandId, // Assuming brandId might be derived or empty
			productsize: data.productSize, // Product Size
			psizecode: data.pSizeCode, // Assuming pSizeCode might be derived or empty
			l1barcode: ' ', // This field is not used in 'All Details' reprint, send blank
		};
	...
	const handleSelectRow = (l1Barcode) => {
		setSelectedRows((prev) =>
			prev.includes(l1Barcode) ? prev.filter((code) => code !== l1Barcode) : [...prev, l1Barcode],
		);
	};
	...
	return (
		<>
			<form onSubmit={handleSubmit(onSubmit)} className="space-y-2 mt-2 mb-2">
									...
									<Select
										...
										onValueChange={(value) => {
											...
											if (selected) {
												...
												machineOptions.unshift({ value: 'ALL', text: 'ALL', disabled: false });
				...
				<div className="grid grid-cols-1 md:grid-cols-3 gap-5">
					...
					<div className="flex flex-col gap-y-2">
						...
						/>
						{errors.brandId && <span className="text-destructive text-sm">{errors.brandId.message}</span>}
					</div>
					<div className="flex flex-col gap-y-2">
						<Controller
							name="productSize"
					...
					<div className="flex flex-col gap-y-2">
						...
						{errors.pCode && <span className="text-destructive text-sm">{errors.pSizeCode.message}</span>}
					</div>
				</div>
			...
			{data && data.length > 0 && (
				<div className="shadow-lg rounded-md w-full max-w-5xl">
