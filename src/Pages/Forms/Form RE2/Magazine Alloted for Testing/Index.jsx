			...
			<form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
									...
									<div className="flex flex-col gap-y-2">
										...
										<Select
											...
											onValueChange={(value) => {
												...
												const selectedPlant = productData?.find(
													(product) => product.ptype === value,
												);
												if (selectedPlant) {
										...
										<SelectContent>
											<SelectGroup>
												{products.map(
													(
														product,
														index, // Changed 'plant' to 'product'
													) => (
														<SelectItem
				...
				<div className="grid grid-cols-4 gap-5">
					<div>
						...
						{errors.testDate && <span className="text-destructive text-sm">{errors.testDate.message}</span>}
					</div>
				</div>
