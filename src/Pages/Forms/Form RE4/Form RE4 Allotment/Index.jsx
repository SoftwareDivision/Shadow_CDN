...
function FormRE4_Allotment() {
    ...
    useEffect(() => {
        if (IndentData) {
            const IndentOptions = [...new Set(IndentData?.map((indent) => indent.indentNo))]
                .sort()
                .map((indentNo) => {
            ...
            const magazineOptions = [...new Set(IndentData?.map((indent) => indent.magName))]
                .sort()
                .map((magName) => ({
                    value: magName,
                    text: magName,
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                                    ...
                                    <div className="flex flex-col gap-y-2">
                                        ...
                                        <Select
                                            ...
                                            onValueChange={(value) => {
                                                ...
                                                const selectedIndent = IndentData?.find(indent => indent.indentNo === value);
                                                if (selectedIndent) {
                                ...
                                render={({ field }) => (
                                    <div className="flex flex-col gap-y-2">
                                        ...
                                        >
                                            ...
                                            <SelectContent>
                                                <SelectGroup>
                                                    {[...new Set(IndentData?.map((item) => item.truckNo))].map((truck) => (
                ...
                <div className="grid grid-cols-2 gap-5">
                    ...
                    <div className="flex flex-col gap-y-2">
                        ...
                        />
                        {errors.re12 && (
                            <span className="text-destructive text-sm">{errors.re12.message}</span>
                        )}
                    </div>
                </div>
